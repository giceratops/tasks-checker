...
export const TaskTableEntry = ({ date, tasks }: TaskTableEntryProps) => {
    ...
    return (<>
        <Tr>
            ...
            <Td>{ weekDays }</Td>
            <Td>{ workDays }</Td>
            <Td>{ countHolidays(date) }</Td>
            <Td><Input type="number" size="xs" value={restDays} onChange={(e) => setRestDays(parseInt(e.target.value))} ></Input></Td>
            <Td>{ timeToWork.toFixed(2) }</Td>
            <Td>{ timeTo }</Td>
            <Td>{ timePeriod01 }</Td>
            <Td>{ timePeriod02 }</Td>
            <Td>{ timeFrom }</Td>
            <Td>{ timeTotal }</Td>
            <Td>{ (timeTotal - timeToWork).toFixed(2) } </Td>
        </Tr>
    </>)
}
